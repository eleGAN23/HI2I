...
def show_image_tensor(x):
    if len(x.shape) == 4:
        ...
        if x.size(0) == 3: # non quat
            ...
            x = x[0:3,:,:]
    plt.imshow(x.cpu().detach().numpy()[0], cmap='gray')
...
def debug_image(nets, args, inputs, step):
    # x_src, y_src = inputs[0][0].to(device), inputs[0][1].to(device)
    # x_ref, y_ref = inputs[1][0].to(device), inputs[1][1].to(device)
    x_src, y_src = inputs.x_src, inputs.y_src
    x_ref, y_ref = inputs.x_ref, inputs.y_ref
    N = x_src.size(0)

    # translate and reconstruct (reference-guided)
    filename = ospj(args.sample_dir, args.experiment_name+'%06d_cycle_consistency.jpg' % (step))
    ...
    y_trg_list = [torch.tensor(y).repeat(N)        for y in range(min(args.num_domains, 5))]
    z_trg_list = torch.randn(args.num_outs_per_domain, 1, l_dim).repeat(1, N, 1)
    for psi in [0.5, 0.7, 1.0]:
        filename = ospj(args.sample_dir, args.experiment_name+'%06d_latent_psi_%.1f.jpg' % (step, psi))
        translate_using_latent(nets, args, x_src, y_trg_list, z_trg_list, psi, filename)

    # reference-guided image synthesis
    filename = ospj(args.sample_dir, args.experiment_name+'%06d_reference.jpg' % (step))
...
def translate_and_reconstruct(nets, args, x_src, y_src, x_ref, y_ref, filename):
    ...
    if not args.real:
        masks = nets.fan.get_heatmap(x_src[:,0:3,:,:]) if args.w_hpf > 0 else None
    else:
...
def video_ref(nets, args, x_src, x_ref, y_ref, fname):
    ...
    for data_next in tqdm(zip(x_ref, y_ref, s_ref), 'video_ref', len(x_ref)):
        ...
        if x_prev.size(1) == 3: # non quat
            entries = entries
        else: #quat
            entries = [x_prev[:,0:3,:,:],x_next[:,0:3,:,:]]
            interpolated = interpolated[:,0:3,:,:]
